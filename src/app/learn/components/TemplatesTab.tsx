...
export default function TemplatesTab() {
  return (
    <div className="w-full pb-12 font-sans">
      <div className="mb-6">
        <h2 className="font-heading text-2xl font-extrabold text-slate-900 mb-1 uppercase tracking-wider">Templates</h2>
        <p className="text-slate-500 text-sm">Document blueprints and workbook templates for your venture.</p>
      </div>

      <Card className="border border-slate-200 rounded-none shadow-none mb-6">
        <CardContent className="p-6">
          <h3 className="font-heading text-slate-900 text-xs uppercase tracking-widest font-bold flex items-center gap-2 mb-4 border-b border-slate-100 pb-3 font-mono">
            Document Blueprints
          </h3>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={idx} className="border border-slate-200 bg-slate-50 p-4 flex flex-col sm:flex-row justify-between sm:items-center rounded-none gap-3">
                <div>
      ...
      <Card className="border border-slate-200 rounded-none shadow-none">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {cardsList.map((card) => (
              <div key={card.id} className="border border-slate-200 rounded-none p-3 bg-slate-50">
                ...
                <p className="text-[11px] text-slate-650 mt-1 leading-relaxed">{card.frontDesc}</p>
                <Badge className="mt-2 text-[10px] bg-[#eae3d7] text-[#5c5346] font-mono tracking-wider font-bold rounded-none border border-amber-250/20">{card.stage}</Badge>
              </div>
            ))}
          </div>
        </CardContent>
